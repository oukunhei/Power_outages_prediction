...
前端允许用户选择 2030–2100 的预测年份，但不提供气候情景、阈值或模型选择。能源项目状态范围和风险模型仍由后端配置固定。
...
生产版本固定只统计 `operating` 项目，因为“装机容量”通常指已经投运的容量。为复现旧项目，保留一个仅供开发测试的旧数据构建命令，但正式页面不使用旧能源输入。

生产页面固定采用 `operating` 口径，且不向用户提供切换按钮。该决定已写入配置和每次运行清单。
...
- 情景：RCP4.5；
- 数据时段：2006–2100；
- 风险输入年份：预计算 2030–2100，并重点发布 2030、2050、2100；
- 多模式处理：对 28 个模型成员取中位数后进行国家级空间聚合；
- 页面展示年份选择器，并在数据来源说明中展示当前年份和 RCP4.5。
...
1. 检查温度和降水单位并统一；
2. 从年度气候指标快照中提取 2030–2100 年；
3. 校验每个模式在每个目标年份均有有效数据；
...
- `power_outage_risk_{2030,2050,2100}.png`：三张共用色标的300 DPI全球风险地图；
...
- 建立回归测试；
- 移除旧 Excel 错误阈值复现，所有正式计算与测试统一使用 HDD > 3000 ℃·日；
- 确认能源状态口径。
...
HDD 阈值确认为大于 3000 ℃·日；正式能源项目范围确认为仅 `operating`。
其余口径为 CMIP5、RCP4.5、预计算 2030–2100、28 模式成员、国家级输出、
默认固定回归模型、预留机器学习接口、Streamlit 单页前端和年份选择器。
